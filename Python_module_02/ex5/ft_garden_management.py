...
class GardenManager:
    ...
    """
	def __init__(self):
		"""Initialize the garden with an empty plant list and a full water tank."""
		self.plants = []
		self.water_tank = 500
	...
	def add_plants(self, plant_name, water, sun):
		...
		try:
			...
			try:
				if water < 0 or sun < 0:
					raise PlantError("Water and Sun numbers, cannot be negative!")
			except Exception:
				raise PlantError("Water and Sun need to be actual numbers!")
		except Exception as e:
			print(f"Error adding plant : {e}")
		else:
			self.plants.append({'name' : plant_name, 'water' : water, 'sun' : sun})
	...
	def check_plant_health(self):
        ...
        Iterates through the plant list and validates their water and sun levels.
		...
		for plant in self.plants:
			try:
				...
				if plant['sun'] < 2:
					raise SunError(f"{plant['name']}: Sunlight hours {plant['sun']} is too low (min 2)")
				elif plant['sun'] > 12:						
					raise SunError(f"{plant['name']}: Sunlight hours {plant['sun']} is too high (max 12)")
