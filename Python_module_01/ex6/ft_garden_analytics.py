...
class FloweringPlant(Plant):
	...
	def __init__(self, name, height, color):
		# super() calls the Parent (Plant) __init__ method to handle basic setup
...
class GardenManager:
	"""
    The main controller class. Manages a collection of plants for a specific owner.
	...
	class GardenStats:
		...
		def calculate_score(plants):
			"""Calculates a custom score: Total Height + Bonus for PrizeFlowers."""
			total = sum(p.height for p in plants)
			for p in plants:
				if type(p) == PrizeFlower:
					# Logic: Prize points are weighted heavily (*4 following the example in the subject)
	...
	def generate_report(self):
		...
		print(f"Plant types: {reg} regular, {flw} flowering, {prize} prize flowers")
...
alice_manager, bob_manager = GardenManager.create_garden_network(["Alice", "Bob"])
...
alice_manager.add_plant(p1)
alice_manager.add_plant(p2)
alice_manager.add_plant(p3)

# 3. Setup Bob's Garden
bob_manager.add_plant(Plant("Bush", 40))
bob_manager.add_plant(FloweringPlant("Daisy", 52, "white"))

# 4. Action: Grow plants
alice_manager.help_plants_grow(1)
...
print(alice_manager.validate_height())

# 7. Scoring Logic
alice_score = alice_manager.GardenStats.calculate_score(alice_manager.plants)
bob_score = bob_manager.GardenStats.calculate_score(bob_manager.plants)
print(f"Garden scores - Alice: {alice_score}, Bob: {bob_score}")

# 8. Print Number of gardens managed (Class variable check)
print(f"Total gardens managed: {alice_manager.total_gardens}")
